...
//					startActivity(intent);
				...
				if(v == sure){
					...
					if(result.equals("success")){
						...
						//发送密码修改成功的action，关闭存在的activity
						final Intent intent = new Intent().setAction("action.passwordChanged");  
						sendBroadcast(intent);
				        Log.i("info","password change action sended");
				        
				        //重启应用
						final Intent i = getPackageManager().getLaunchIntentForPackage(getPackageName());  
