...
public class DepositService {
	...
	public boolean addDeposit(Context ctx, int bankID, String depositName,
		...
		hs.addHappyBi(ctx, Config.DEPOSIT_GENERATE_HAPPYBI);
		...
		Comparator<Deposit> comparator = new Comparator<Deposit>() {
			public int compare(Deposit d1, Deposit d2) {
				if (d1.getMood() != d2.getMood()) {// 先排心情
					...
					return d2.getDepositID() - d1.getDepositID();
				}
			}
		};
	...
	public int getInterval(Context ctx) {
		int minInterval = 0;
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		...
		c.moveToLast();//统计刚插入的存款信息
		if (c.getInt(c.getColumnIndex("mood")) == 0){
			return 0;
		} else {
			try {
				list.add(sdf.parse(c.getString(c.getColumnIndex("deposittime"))));
			} catch (ParseException e) {
			}
		}
